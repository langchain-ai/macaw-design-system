  ...
  try {
    ...
    expect(JSON.parse(discovered.stdout)).toEqual([
      expect.objectContaining({
        name: 'NewControl',
        importPath: '@langchain/macaw-components/components/NewControl',
        stories: [],
      }),
    ]);
    ...
    expect(JSON.parse(listed.stdout)).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          name: 'DefaultOnly',
          importStatement:
            "import { DefaultOnly } from '@langchain/macaw-components/components/DefaultOnly';",
        }),
      ])
    );
...
test.each([
  ...
  (name, family, source) => {
    ...
    expect(JSON.parse(result.stdout)).toMatchObject({
      name,
      family,
      importPath: `@langchain/macaw-components/components/${family}`,
      importStatement: `import { ${name} } from '@langchain/macaw-components/components/${family}';`,
...
test('discovers current components and returns usable imports and source paths', () => {
  ...
  for (const component of components) {
    const modulePath = path.join(
      root,
      component.importPath.replace('@langchain/macaw-components/', 'src/')
    );
    expect(
...
test('provides the correct default import for UnsavedChangesDialog', () => {
  ...
  expect(JSON.parse(result.stdout)).toMatchObject({
    importStatement:
      "import UnsavedChangesDialog from '@langchain/macaw-components/components/UnsavedChangesDialog';",
  });
});
...
test('keeps component as a compatible alias for inspect', () => {
  ...
  expect(JSON.parse(result.stdout)).toMatchObject({
    name: 'Typeahead',
    importPath: '@langchain/macaw-components/components/Typeahead',
  });
...
test('handles unquoted queries, empty results, and text output', () => {
  ...
  expect(search.stdout).toContain(
    '@langchain/macaw-components/components/Typeahead'
  );
...
test('looks up components case-insensitively without unrelated stories', () => {
  ...
  expect(JSON.parse(result.stdout)).toMatchObject({
    name: 'Button',
    importPath: '@langchain/macaw-components/components/Button',
    stories: ['src/components/Button/Button.stories.tsx'],
  });
